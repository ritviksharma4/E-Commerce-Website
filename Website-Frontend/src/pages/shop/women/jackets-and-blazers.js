...
const JacketsBlazersWomenPage = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const storageKey = "velvet_login_key.filters.women.jacketsAndBlazers";
      try {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('jacketsBlazers_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'women', subCategory: 'jackets-and-blazers' }),
      });
      ...
      console.error('Error fetching jackets-and-blazers from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.women.jacketsAndBlazers";
    ...
    const detailedFilters = userObj.filters?.women?.jacketsAndBlazers || [];
    ...
    userObj.filters.women.jacketsAndBlazers = detailedFilters;
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/women', label: 'Women' }, { label: 'Jackets & Blazers' }]}
                />
              </div>
            </Container>
            <Banner
              ...
              subtitle={
                "From tailored blazers to edgy puffers, explore a curated collection of women’s jackets—where contemporary cuts, luxe textures, and timeless silhouettes redefine outerwear with bold flair and effortless charm."
              }
            />
            <Container size={'large'} spacing={'min'}>
              ...
              <CardController
                ...
                categoryKey="women"
                subcategoryKey="jacketsAndBlazers"
              />
