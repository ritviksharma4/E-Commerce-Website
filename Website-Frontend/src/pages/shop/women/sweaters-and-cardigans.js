...
const SweatersCardigansWomenPage = () => {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('sweatersCardigans_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'women', subCategory: 'sweaters-and-cardigans' }),
      });
      ...
      console.error('Error fetching sweaters-and-cardigans from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/women', label: 'Women' }, { label: 'Sweaters & Cardigans' }]}
                />
              </div>
            </Container>
            <Banner
