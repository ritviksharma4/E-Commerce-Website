...
import * as styles from './dresses.module.css';
...
const DressesWomenPage = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const storageKey = "velvet_login_key.filters.women.dresses";
      try {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('dresses_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'women', subCategory: 'dresses' }),
      });
      ...
      console.error('Error fetching dresses from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('dresses_loadedItemCount', newCount);
  };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.women.dresses";
    ...
    const detailedFilters = userObj.filters?.women?.dresses || [];
    ...
    userObj.filters.women.dresses = detailedFilters;
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/women', label: 'Women' }, { label: 'Dresses' }]}
                />
              </div>
            </Container>
            <Banner
              ...
              subtitle={
                "Look to our Women’s Dresses for modern takes on one-and-done dressing. From midis in bold prints to dramatic floor-sweeping styles and easy all-in-ones, our edit covers every mood."
              }
            />
            <Container size={'large'} spacing={'min'}>
...
export default DressesWomenPage;