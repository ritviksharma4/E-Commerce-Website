...
const BagsAccessoriesPage = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const storageKey = "velvet_login_key.filters.accessories.bags";
      try {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('bagsAccessories_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'accessories', subCategory: 'bags' }),
      });
      ...
      console.error('Error fetching bags from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('bagsAccessories_loadedItemCount', newCount);
  };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.accessories.bags";
    ...
    const detailedFilters = userObj.filters?.accessories?.bags || [];
    ...
    userObj.filters.accessories.bags = detailedFilters;
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/accessories', label: 'Accessories' }, { label: 'Bags' }]}
                />
              </div>
            </Container>
            <Banner
              ...
              name={`Bags`}
              subtitle={
                "Carry style with ease—discover statement handbags, versatile backpacks, sleek duffles, and everyday totes, all crafted to blend utility with elegance for every journey, occasion, and outfit."
              }
            />
            <Container size={'large'} spacing={'min'}>
