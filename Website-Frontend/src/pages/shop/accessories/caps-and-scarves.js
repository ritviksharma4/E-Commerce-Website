...
import * as styles from './caps-and-scarves.module.css';
...
const CapsScarvesAccessoriesPage = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const storageKey = "velvet_login_key.filters.accessories.capsAndScarves";
      try {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('capsScarves_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'accessories', subCategory: 'caps-and-scarves' }),
      });
      ...
      console.error('Error fetching caps & scarves from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('capsScarves_loadedItemCount', newCount);
  };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.accessories.capsAndScarves";
    ...
    const detailedFilters = userObj.filters?.accessories?.capsAndScarves || [];
    ...
    if (!userObj.filters.accessories) userObj.filters.accessories = {};
    userObj.filters.accessories.capsAndScarves = detailedFilters;
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/accessories', label: 'Accessories' }, { label: 'Caps & Scarves' }]}
                />
              </div>
            </Container>
            <Banner
              ...
              name={`Caps & Scarves`}
              subtitle={
                "Top off your look with flair—explore sleek caps and cozy scarves that blend fashion and function, adding the perfect finishing touch to every outfit, season, and style statement."
              }
            />
            <Container size={'large'} spacing={'min'}>
              ...
              <CardController
                ...
                subcategoryKey="caps-and-scarves"
              />
...
export default CapsScarvesAccessoriesPage;