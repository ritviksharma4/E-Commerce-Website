...
const TShirtsShirtsMenPage = () => {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('tShirtsShirtsMen_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'men', subCategory: 't-shirts-and-shirts' }),
      });
      ...
      console.error('Error fetching t-shirts-and-shirts from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/men', label: 'Men' }, { label: 'T-Shirts & Shirts' }]}
                />
              </div>
            </Container>
            <Banner
