...
import * as styles from './trousers.module.css';
...
const TrousersMenPage = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const storageKey = "velvet_login_key.filters.men.trousers";
      try {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('trousersMen_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'men', subCategory: 'trousers' }),
      });
      ...
      console.error('Error fetching trousers from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('trousersMen_loadedItemCount', newCount);
  };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.men.trousers";
    ...
    const detailedFilters = userObj.filters?.men?.trousers || [];
    ...
    userObj.filters.men.trousers = detailedFilters;
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/men', label: 'Men' }, { label: 'Trousers' }]}
                />
              </div>
            </Container>
            <Banner
              ...
              name={`Men's Trousers`}
              subtitle={
                "Step into refined comfort with trousers that fuse function and form—think tailored fits, relaxed cargos, and modern cuts designed for everyday ease, urban edge, and timeless versatility."
              }
            />
            <Container size={'large'} spacing={'min'}>
...
export default TrousersMenPage;