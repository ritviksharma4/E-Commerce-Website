...
const JacketsMenPage = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const storageKey = "velvet_login_key.filters.men.jackets";
      try {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('jackets_men_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'men', subCategory: 'jackets' }),
      });
      ...
      console.error('Error fetching jackets from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('jackets_men_loadedItemCount', newCount);
  };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.men.jackets";
    ...
    const detailedFilters = userObj.filters?.men?.jackets || [];
    ...
    userObj.filters.men.jackets = detailedFilters;
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/men', label: 'Men' }, { label: 'Jackets' }]}
                />
              </div>
            </Container>
            <Banner
              ...
              name={`Jackets`}
              subtitle={
                "Elevate your outerwear game with rugged leather, sporty silhouettes, and modern zip-ups—crafted for comfort, style, and versatility that moves seamlessly from street to summit."
              }
            />
            <Container size={'large'} spacing={'min'}>
