...
const JacketsMenPage = () => {
  ...
  const restoreScroll = () => {
    const scrollY = sessionStorage.getItem('jacketsMen_scrollY');
    if (scrollY) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error fetching men’s jackets from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  useEffect(() => {
    const storeScroll = () => {
      sessionStorage.setItem('jacketsMen_scrollY', window.scrollY.toString());
    };
