...
import * as styles from './heels-and-sandals.module.css';
...
const HeelsSandalsFootwearPage = () => {
  ...
  const restoreScroll = () => {
    const scrollY = sessionStorage.getItem('heelsSandals_scrollY');
    if (scrollY) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    const savedIndex = parseInt(sessionStorage.getItem('heelsSandals_loadedItemCount')) || ITEMS_PER_PAGE;
    ...
    try {
      ...
      console.error('Error fetching heels & sandals products from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('heelsSandals_loadedItemCount', newCount);
  };
  ...
  useEffect(() => {
    const storeScroll = () => {
      sessionStorage.setItem('heelsSandals_scrollY', window.scrollY.toString());
    };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.footwear.heelsAndSandals";
    ...
    const detailedFilters = userObj.filters?.footwear?.heelsAndSandals || [];
    ...
    userObj.filters.footwear.heelsAndSandals = detailedFilters;
        ...
        {loading || filtering ? (
          ...
          <>
            ...
            <Banner
              ...
              name={`Heels & Sandals`}
              subtitle={
                'Indulge in the ultimate expression of elegance with our curated collection of women’s heels and sandals. Each design, from classic stilettos to contemporary wedges, is crafted with the finest materials and impeccable craftsmanship. Perfectly balanced for style and comfort, these shoes are made to elevate any occasion, adding a touch of glamour to your every step.'
              }
            />
            <Container size={'large'} spacing={'min'}>
              ...
              <CardController
                ...
                subcategoryKey="heelsAndSandals"
              />
...
export default HeelsSandalsFootwearPage;