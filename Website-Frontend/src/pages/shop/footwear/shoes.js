...
const ShoesFootwearPage = () => {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('shoes_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'footwear', subCategory: 'shoes' }),
      });
      ...
      console.error('Error fetching shoes from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/footwear', label: 'Footwear' }, { label: 'Shoes' }]}
                />
              </div>
            </Container>
            <Banner
