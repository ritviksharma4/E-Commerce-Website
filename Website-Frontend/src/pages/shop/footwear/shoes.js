...
import * as styles from './shoes.module.css';
...
const ShoesFootwearPage = () => {
  ...
  const [savedFilters, setSavedFilters] = useState(() => {
    const storageKey = "velvet_login_key.filters.footwear.shoes";
  ...
  const restoreScroll = () => {
    const scrollY = sessionStorage.getItem('shoes_scrollY');
    if (scrollY) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    const savedIndex = parseInt(sessionStorage.getItem('shoes_loadedItemCount')) || ITEMS_PER_PAGE;
    ...
    try {
      ...
      console.error('Error fetching shoes products from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('shoes_loadedItemCount', newCount);
  };
  ...
  useEffect(() => {
    const storeScroll = () => {
      sessionStorage.setItem('shoes_scrollY', window.scrollY.toString());
    };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.footwear.shoes";
    ...
    const detailedFilters = userObj.filters?.footwear?.shoes || [];
    ...
    userObj.filters.footwear.shoes = detailedFilters;
...
export default ShoesFootwearPage;