...
import * as styles from './footwear.module.css';
...
const AllFootwearPage = () => {
  ...
  const [savedFilters, setSavedFilters] = useState(() => {
    const storageKey = "velvet_login_key.filters.footwear.allFootwear";
  ...
  const restoreScroll = () => {
    const scrollY = sessionStorage.getItem('footwear_scrollY');
    if (scrollY) {
  ...
  const fetchProducts = useCallback(async () => {
    const savedIndex = parseInt(sessionStorage.getItem('all_footwear_loadedItemCount')) || ITEMS_PER_PAGE;
    ...
    try {
      ...
      console.error('Error fetching footwear from Lambda:', error);
    } finally {
      setLoading(false);
    }
  }, [LAMBDA_ENDPOINT]);
  ...
  const handleLoadMore = () => {
    ...
    sessionStorage.setItem('all_footwear_loadedItemCount', newCount);
  };
  ...
  useEffect(() => {
    const storeScroll = () => {
      sessionStorage.setItem('footwear_scrollY', window.scrollY.toString());
    };
  ...
  const handleRemoveFilter = (filterCategory, filterName) => {  
    const storageKeySimple = "velvet_login_key.filters.footwear.allFootwear";
    ...
    const detailedFilters = userObj.filters?.footwear?.allFootwear || [];
    ...
    if (!userObj.filters.footwear) userObj.filters.footwear = {};
    userObj.filters.footwear.allFootwear = detailedFilters;
        ...
        {loading || filtering ? (
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[
                    { link: '/', label: 'Home' },
                    { link: '/shop/footwear', label: 'Footwear' },
                    { label: 'All Footwear' },
                  ]}
                />
              </div>
            ...
            <Banner
              ...
              name={`All Footwear`}
              subtitle={
                'Discover the epitome of sophistication with our exclusive collection of footwear. From elegant heels to versatile sandals and unisex designs, each pair is crafted with precision and luxury in mind. Meticulously sourced materials, innovative comfort features, and timeless silhouettes ensure that every step you take radiates style and confidence. Elevate your wardrobe with footwear that effortlessly blends fashion, function, and luxury.'
              }
            />
            <Container size={'large'} spacing={'min'}>
...
export default AllFootwearPage;