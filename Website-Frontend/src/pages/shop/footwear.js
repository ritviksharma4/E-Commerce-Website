...
const AllFootwearPage = () => {
  ...
  const getLoadedItemCount = () => {
    return parseInt(sessionStorage.getItem('all_footwear_loadedItemCount')) || ITEMS_PER_PAGE;
  };
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(LAMBDA_ENDPOINT, {
        ...
        body: JSON.stringify({ email, category: 'footwear' }),
      });
          ...
          <>
            <Container size={'large'} spacing={'min'}>
              <div className={styles.breadcrumbContainer}>
                <Breadcrumbs
                  crumbs={[{ link: '/', label: 'Home' }, { link: '/shop/footwear', label: 'Footwear' }, { label: 'All Footwear' }]}
                />
              </div>
            </Container>
            <Banner
